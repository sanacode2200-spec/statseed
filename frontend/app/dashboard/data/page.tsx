...
function ColTypeBadge({ dtype }: { dtype: ColumnInfo["dtype"] }) {
  return dtype === "continuous" ? (
    ...
    <span className="inline-block rounded px-1.5 py-0.5 text-xs font-medium bg-gray-100 text-gray-600">
      カテゴリ
    </span>
  );
}
...
export default function DataPage() {
  ...
  return (
    <div>
      <h1 className="text-xl font-bold text-gray-800 mb-1">データ読み込み</h1>
      ...
      <div
        ...
        className={`mb-6 flex flex-col items-center justify-center gap-2 rounded-xl border-2 border-dashed cursor-pointer transition-colors px-6 py-12 ${
          dragging
      ...
      >
        ...
        <p className="text-xs text-gray-400">CSV・Excel（.xlsx / .xls）対応 / 最大10MB</p>
        <input
          ref={inputRef}
          type="file"
          accept={ACCEPT}
        ...
        <div className="space-y-6">
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-200 text-left">
                    <th className="pb-2 pr-4 font-medium text-gray-600">列名</th>
                    <th className="pb-2 pr-4 font-medium text-gray-600">型</th>
                    <th className="pb-2 pr-4 font-medium text-gray-600 text-right">有効数</th>
                    <th className="pb-2 pr-4 font-medium text-gray-600 text-right">欠損数</th>
                    ...
                    <tr
                      ...
                      className={`border-b border-gray-100 last:border-0 transition-colors ${
                        col.dtype === "continuous"
                          ...
                          : "opacity-60"
                      } ${activeCol === col.name ? "bg-blue-50" : ""}`}
                    >
                      <td className="py-2 pr-4 font-medium text-gray-800">
                        {col.name}
                        {activeCol === col.name && (
                          <span className="ml-2 text-xs text-blue-500">▶ 選択中</span>
                        )}
                      </td>
                      ...
                      <td className="py-2 pr-4 text-right font-mono text-gray-500">
                        {col.n_missing > 0 ? (
                          <span className="text-orange-600">{col.n_missing}</span>
                        ) : (
                          "0"
                        )}
                      </td>
          ...
          {activeColInfo && (
            <Card>
              ...
              <div className="font-mono text-sm bg-gray-50 rounded-md p-3 max-h-48 overflow-y-auto leading-relaxed text-gray-700">
                ...
                {activeColInfo.values.length > 50 && (
                  <span className="text-gray-400">... 他 {activeColInfo.values.length - 50} 件</span>
                )}
              </div>
            </Card>
          )}
                ...
                <thead>
                  <tr className="border-b border-gray-200">
                    {result.columns.map((col) => (
                      <th
                        key={col.name}
                        className="pb-2 pr-4 text-left font-medium text-gray-600 whitespace-nowrap"
                      >
                        {col.name}
                      </th>
                    ))}
                      ...
                      {result.columns.map((col) => (
                        <td
                          ...
                          className={`py-1.5 pr-4 font-mono whitespace-nowrap ${
                            row[col.name] === null ? "text-orange-400" : "text-gray-700"
                          }`}
                        >
                          {row[col.name] ?? "NA"}
                        </td>
