...
import { continuousColumns, findColumn } from "@/lib/dataUtils";
...
export default function RegressionPage() {
  ...
  async function run() {
    ...
    setLoading(true);
    try {
      let req: LinearRegressionRequest;
      ...
      const res = await api.regressionLinear(req);
      setResult(res);
    } catch (e) {
  ...
  return (
    <div className="max-w-5xl">
      <div className="mb-5">
        <h1 className="text-[20px] font-semibold text-gray-900 dark:text-white">回帰分析（線形回帰）</h1>
        <p className="text-[13px] text-gray-400 dark:text-neutral-500 mt-1">
          1つの目的変数を1つ以上の説明変数で予測します。説明変数を複数指定すると重回帰（共変量調整）になります。
        </p>
      </div>
        ...
        <div className="space-y-3">
          <Card className="p-4">
            <div className="mb-3 flex flex-wrap items-center gap-2">
              <span className="text-[12px] text-gray-500 dark:text-neutral-500 shrink-0">目的変数</span>
              <input
            ...
            <textarea
              ...
              rows={3}
              placeholder="目的変数の数値をスペース・改行・カンマ区切りで入力（欠損: NA, -）"
              value={outcomeText}
